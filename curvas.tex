...
\begin{pspicture}(-3,-1)(3,3.0)
%\psframe*(-3,-3)(3.5,3.5)
\psset{viewpoint=20 20 13,Decran=15,lightsrc=20 10 5}
\defFunction[algebraic]%
{helice}(t)
{5*cos(2*t)}{5*sin(2*t)}{t}
...
resolution=360,
function=helice]%
\axesIIID[linecolor=black](0,0,0)(6,6,7)
 \end{pspicture}  
  \caption{\label{helicedex}Função vetorial associada à uma hélice circular dextrogira.}
  \vspace{-1.2cm}
\end{wrapfigure}
...
\begin{prob} Reconheça e represente graficamente as curvas descritas pelas seguintes funções vetoriais:
\begin{itemize}
...
\end{proof}
\begin{prob} Dada a função vetorial $\vct{r}(t)=t^2\vct{i}+e^t\vct{j}-2\cos\pi t ~\! \vct{k}$, calcule:
\begin{itemize}
...
\begin{prob} Verifique que a função vetorial dada por $\vct{f}(t)=\frac{1-t^2}{1+t^2}\vct{i}+\frac{2t}{1+t^2}\vct{j}$, $-\infty<t<\infty$
...
\begin{prob}Encontre a derivada de cada uma das funções vetoriais do exemplo \ref{exfv1}

Resp: $\vct{f'}(t)=\cos(t)\vct{i}-\sin(t)\vct{j}$,~~ $\vct{g'}(t)= \vct{i}+\sinh(t)\vct{k}$, ~~$\vct{h'}(t)=\cos(t)\vct{i}-\sin(t)\vct{j}+\vct{k}$

\end{prob}

\begin{prob} Mostre as seguintes identidades:
\begin{itemize}
...
\psSolid[object=point,args=1.92 -1.4 3,name=P1]
   \psSolid[object=point,args=1.51 3.28 3.5,name=P2]
    \psSolid[object=point,args=-0.29 4.95 4,name=P3]
    \psSolid[object=point,args=-1.82 2.06 4.5,name=P4]
     \psSolid[object=point,args=-1.68 -2.72 5,name=P5]
      \psSolid[object=point,args=0.01 -5 5.5,name=P6]
       \psSolid[object=point,args=1.69 -2.68 6,name=P7]
        \psSolid[object=point,args=1.81 2.1 6.5,name=P8]
         \psSolid[object=point,args=0.27 4.95 7,name=P9]
         \psSolid[object=point,args=-1.51 3.25 7.5,name=P10]
         \psSolid[object=point,args=-1.91 -1.44 8,name=P11]
   \psSolid[object=line,linecolor=red!50,linewidth=2pt,args=P1 P2]
   \psSolid[object=line,linecolor=red!50,linewidth=2pt,args=P2 P3]
   \psSolid[object=line,linecolor=red!50,linewidth=2pt,args=P3 P4]
   \psSolid[object=line,linecolor=red!50,linewidth=2pt,args=P4 P5]
   \psSolid[object=line,linecolor=red!50,linewidth=2pt,args=P5 P6]
   \psSolid[object=line,linecolor=red!50,linewidth=2pt,args=P6 P7]
   \psSolid[object=line,linecolor=red!50,linewidth=2pt,args=P7 P8]
   \psSolid[object=line,linecolor=red!50,linewidth=2pt,args=P8 P9]
   \psSolid[object=line,linecolor=red!50,linewidth=2pt,args=P9 P10]
   \psSolid[object=line,linecolor=red!50,linewidth=2pt,args=P10 P11]      
   
   \psPoint(1.92,-1.4,2){P01}
   \psdot(P01)
   \rput*(P01){$P_0$}
   \psPoint(1.51,3.28,2.5){P02}
   \psdot(P02)
   \rput*(P02){$P_1$}

   
      \psPoint(1.69,-2.68,5){P07}
   \psdot(P07)
   \rput*(P07){$P_i$}
   \psPoint(1.81,2.1,5.5){P08}
   \psdot(P08)
   \rput*(P08){$P_{i+1}$}

   
%\axesIIID[linecolor=black](-3,0,0)(3,6,7)
 \end{pspicture}\caption{Aproximação poligonal do comprimento do arco}\label{fig_compr_arc}
\end{wrapfigure}
...
\end{wrapfigure}
\begin{prob}Calcule a curvatura da curva $\vct{r}=a\cos(t)\vct{i}+a\sin(t)\vct{j},\qquad a>0,\ 0\leq t\leq 2\pi$.
\end{prob}
...
\begin{prob} Calcule a curvatura, o raio de curvatura e o módulo da torção das curvas abaixo:
\begin{itemize}
...
\begin{prob}{\label{prob_torcao}} Dada a hélice circular $\vct{r}=a\cos(t)\vct{i}+a\sin(t)+ct\vct{k}$, $t\geq 0$, $a>0,\ c>0$, calcule o valor de $c$ para que a torção seja máxima.
\end{prob}
...
\end{obs}
\begin{prob}Calcule curvatura e torção para a curva $\vct{r}=a\cos(t)\vct{i}+a\sin(t)\vct{j}+ct\vct{k}$, $a>0$, $c>0$, $t>0$ usando as expressões (\ref{curvatura_2}) e (\ref{torcao_2}).
\end{prob}
